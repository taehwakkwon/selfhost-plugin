...
- Gateway: `https://gateway.example.com/v1/chat/completions` — vLLM instances behind sgl-router (SGLang Model Gateway). Editable, not hardcoded.
...
lib/opencode.mjs — single execution path for rescue AND review
        ...
        ├─ registers gateway.example.com as a custom OpenAI-compatible provider
