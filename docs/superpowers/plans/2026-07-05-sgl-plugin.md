...
- Gateway: `https://gateway.example.com/v1` (OpenAI-compatible chat completions), editable — never hardcoded outside `lib/config.mjs`'s default-seed value.
...
Everything through Task 15 is either unit-tested or a static asset. This task is the one place the plan intentionally does not prescribe automated tests: it exercises `opencode` actually installed, a real network round-trip to `https://gateway.example.com/v1`, and Claude Code's own plugin-loading mechanism — none of which are meaningfully mockable without testing the mock instead of reality. Run every step below and note the actual outcome; do not mark this task done from inspection alone.
